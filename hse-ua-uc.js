...
function normalize(data) {
  ...
  const insights = (data.insights || []).filter(
    (i) => !/sheet 2|good obs/i.test(i)
  );
  return { ...data, ua, insights };
}
...
function drawGroupedBars(canvas, groups, series, colors) {
  ...
  groups.forEach((label, gi) => {
    ...
    const short = narrow && label.length > 6 ? label.replace("'", "\n") : label;
    if (narrow && short.includes("\n")) {
      short.split("\n").forEach((line, li) => ctx.fillText(line, gx, h - 22 + li * 10));
    } else {
...
function renderBanner(d) {
  document.getElementById("hse-title").textContent = "UA/UC Deep Dive";
  document.getElementById("hse-subtitle").textContent =
    `${d.period} · ${d.field} · Sheet 1 — UA/UC`;
  const ua = d.ua;
  document.getElementById("hse-banner").innerHTML = `
    <div class="gas-banner-grid">
      <div class="gas-banner-item"><span class="gas-banner-label">Records</span><span class="gas-banner-value">${fmt(ua.total)} UA/UC</span></div>
      ...
      <div class="gas-banner-item"><span class="gas-banner-label">Open</span><span class="gas-banner-value">${fmt(ua.status?.Open || 0)}</span></div>
      <div class="gas-banner-item"><span class="gas-banner-label">Overdue</span><span class="gas-banner-value">${fmt(ua.overdue_open || 0)}</span></div>
    </div>`;
}
...
function renderKpi(d) {
  ...
  document.getElementById("hse-kpi").innerHTML = `
    <article class="gas-kpi-card gas-kpi-raw">
      <div class="gas-kpi-head"><h3 class="gas-kpi-title">UA/UC Register</h3><p class="gas-kpi-loc">${fmt(u.total)} reports · ${d.closure_rate_pct}% closed</p></div>
    ...
    <article class="gas-kpi-card gas-kpi-permeate">
      <div class="gas-kpi-head"><h3 class="gas-kpi-title">Risk &amp; SWA</h3><p class="gas-kpi-loc">Sheet 1 breakdown</p></div>
      <dl class="gas-kpi-metrics">
        ...
        <div class="gas-kpi-metric"><dt>In Progress</dt><dd>${fmt(u.status?.["In Progress"] || 0)}</dd></div>
      </dl>
    </article>
    <article class="gas-kpi-card gas-kpi-product">
      <div class="gas-kpi-head"><h3 class="gas-kpi-title">Hazard Profile</h3><p class="gas-kpi-loc">From descriptions</p></div>
      <dl class="gas-kpi-metrics">
        <div class="gas-kpi-metric primary"><dt>PSF Flagged</dt><dd>${fmt(d.hazard_analysis?.psf_applicable_count || 0)}</dd></div>
        <div class="gas-kpi-metric"><dt>Medium Risk Events</dt><dd>${fmt(d.hazard_analysis?.medium_risk_total || 0)}</dd></div>
        <div class="gas-kpi-metric"><dt>Top Dept</dt><dd class="hse-kpi-dept">${topEntries(u.department, 1)[0]?.[0] || "—"}</dd></div>
      </dl>
    </article>`;
}
...
function renderCombined(d) {
  const mon = topEntries(d.ua.monitoring, 8);
  document.getElementById("hse-monitoring").innerHTML = `<table class="results-table"><thead><tr><th>Point</th><th class="gas-num-cell">Count</th><th class="gas-num-cell">%</th></tr></thead><tbody>
...
function renderHazard(d) {
  ...
  drawHorizontalBars(document.getElementById("hazard-theme-chart"), topEntries(h.hazard_themes, 10), "#ef4444");
  drawHorizontalBars(document.getElementById("hazard-area-chart"), Object.entries(h.area_groups || {}).sort((a, b) => b[1] - a[1]), "#f97316");
  ...
  document.getElementById("hazard-open-golden").innerHTML = `<table class="results-table"><thead><tr>
    ...
    ${topEntries(h.open_by_golden, 8).map(([k, v]) => `<tr><td>${k}</td><td class="gas-num-cell">${fmt(v)}</td></tr>`).join("")}
    </tbody></table>`;
...
function renderAll(raw) {
  ...
  renderSheet1(d); renderCombined(d); renderHazard(d);
}
