...
function drawGroupedBars(canvas, groups, series, colors) {
  ...
  const barW = Math.min(22, groupW / (series.length + 1));
  groups.forEach((label, gi) => {
    ...
    ctx.fillStyle = "#e2e8f0"; ctx.font = "10px system-ui,sans-serif"; ctx.textAlign = "center";
...
function drawHorizontalBars(canvas, entries, color = "#2563eb") {
  ...
  const { ctx, w, h } = setupCanvas(canvas, canvas.getAttribute("height") | 0 || 300);
  const pad = { top: 10, right: 50, bottom: 10, left: 140 };
  const rowH = Math.min(26, (h - pad.top - pad.bottom) / entries.length);
  const plotW = w - pad.left - pad.right;
  ...
  entries.forEach(([label, val], i) => {
    ...
    ctx.fillStyle = "#e2e8f0"; ctx.font = "11px system-ui,sans-serif"; ctx.textAlign = "right";
    ctx.fillText(label.length > 20 ? label.slice(0, 18) + "…" : label, pad.left - 8, y + rowH * 0.65);
    ctx.fillStyle = color; ctx.fillRect(pad.left, y + 3, bw, rowH - 6);
    ctx.fillStyle = "#f8fafc"; ctx.textAlign = "left";
    ctx.fillText(fmt(val), pad.left + bw + 6, y + rowH * 0.65);
  });
}
...
function drawDonut(canvas, segments) {
  ...
  ctx.fillStyle = "#fbbf24"; ctx.font = "bold 22px system-ui,sans-serif"; ctx.textAlign = "center";
  ctx.fillText(fmt(total), cx, cy + 4);
  ctx.fillStyle = "#e2e8f0"; ctx.font = "11px system-ui,sans-serif"; ctx.fillText("Total", cx, cy + 22);
  let ly = h - 8;
  segments.forEach(([label, val, color]) => {
    ctx.fillStyle = color; ctx.fillRect(16, ly - 10, 10, 10);
    ctx.fillStyle = "#f1f5f9"; ctx.font = "12px system-ui,sans-serif"; ctx.textAlign = "left";
